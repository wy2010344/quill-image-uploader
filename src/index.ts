...
export type ImageUploaderOption = {
  upload(bolb: Blob): Promise<string>
}
export default class ImageUploader {
  quill: Quill;
  options: ImageUploaderOption
  range: null | RangeStatic
  fileHolder: HTMLInputElement | undefined;
  constructor(quill: Quill, options: ImageUploaderOption) {
    this.quill = quill;
  ...
  handleDrop(evt: any) {
    ...
    if (
      ...
      if (document.caretRangeFromPoint) {
        ...
        const selection = document.getSelection();
        const range = (document as any).caretPositionFromPoint(evt.clientX, evt.clientY);
        if (selection && range) {
          selection.setBaseAndExtent(
            range.offsetNode,
            range.offset,
